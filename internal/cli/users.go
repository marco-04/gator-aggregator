...
func doCallbackLoggedIn(callback func (s *state.State, args []string, user database.User) error) func(*state.State, []string) error {
	return func (s *state.State, args []string) error {
		user, err := s.DB.GetCurrentUser(context.Background(), s.Config.CurrentUserName)
		if err != nil {
			return fmt.Errorf("db error: %w", err)
		}

		return callback(s, args, user)
	}
}
